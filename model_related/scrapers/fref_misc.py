...
class Scraper:
    ...
    def extract_table_data(self,url,headers_list, season):
        ...
        headers = [th.get('data-stat') for th in player_stats[0].find('div',id = re.compile('div_stats_.*_misc')).find('thead').find_all('tr')[1].find_all('th')]
        ...
        for i in player_stats:
            i = i.find('div',id = re.compile('div_stats_.*_misc'))
...
for i in scraper.matches_all_season:
    season = i["season"]
    print(colored(f'Start scraping misc stats for season {season}', 'cyan'))
    matches = i['matches']
    ...
    print(colored(f'Finished scraping misc stats for season {season}', 'green'))