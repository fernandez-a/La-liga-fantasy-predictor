...
class Scraper:
    ...
    def extract_table_data(self,url,headers_list, season):
        ...
        player_stats = soup.find_all('div', id=re.compile('div_stats_.*_passing')) 
        ...
        for i in player_stats:
            for tr in player_stats.find('tbody').find_all('tr'):
                row = [tr.find('th').text]
...
# matches = scraper.getMatches(seasons)
...
for i in scraper.matches_all_season:
    ...
    for chunk_index, chunk in enumerate(match_chunks):
        filename = f"data/{season}/{season}_chunk_{chunk_index}.csv"
