...
class Scraper:
    ...
    def extract_table_data(self,url,headers_list, season):
        ...
        headers = [th.get('data-stat') for th in player_stats[0].find('div',id = re.compile('div_stats_.*_passing_type')).find('thead').find_all('tr')[1].find_all('th')]
        ...
        for i in player_stats:
            i = i.find('div',id = re.compile('div_stats_.*_passing_type'))
...
for i in scraper.matches_all_season:
    season = i["season"]
    print(colored(f'Start scraping passing-type stats for season {season}', 'cyan'))
    matches = i['matches']
    ...
    for chunk_index, chunk in enumerate(match_chunks):
        filename = f"./all_seasons_data/{season}/{season}_chunk_{chunk_index}_passing_type.csv"
    ...
    print(colored(f'Finished scraping passing-type for season {season}', 'green'))