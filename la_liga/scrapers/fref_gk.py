    ...
    def extract_table_data(self,url, season):
        ...
        for i in player_stats:
            ...
            for tr in i.find('tbody').find_all('tr'):
                ...
                row.append(season)
                row.append(i.find('caption').text.split('Goalkeeper')[0])
                rows.append(row)
...
for i in scraper.matches_all_season:
    season = i["season"]
    print(colored(f'Start scraping goalkeepers stats for season {season}', 'cyan'))
    matches = i['matches']
    ...
    for chunk_index, chunk in enumerate(match_chunks):
        filename = f"./all_seasons_data/{season}/{season}_chunk_{chunk_index}_goalkeepers.csv"
    ...
    print(colored(f'Finished scraping goalkeepers stats for season {season}', 'green'))